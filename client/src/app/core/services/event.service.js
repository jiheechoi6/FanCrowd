...
let EventService = class EventService {
    ...
    getEventsByCategoryAndFandom(categoryName, fandomName) {
        // Get events from server, code below requires server call
        let filterdEvents = [];
        this.events.forEach((event) => {
            var _a, _b;
            if (((_a = event.fandomType.category) === null || _a === void 0 ? void 0 : _a.toLowerCase()) ===
                categoryName.toLowerCase() &&
                ((_b = event.fandomType.name) === null || _b === void 0 ? void 0 : _b.toLowerCase().split(' ').join('-')) ===
    ...
    updateEventAttendance(id, isAttending) {
    ...
    getReviewsByEventId(eventId) {
        ...
        var _a;
        return ((_a = this.events.find((event) => event.id === eventId)) === null || _a === void 0 ? void 0 : _a.reviews) || [];
    }
    ...
    updateReviewById(eventId, updatedReview) {
    ...
    deleteReview(eventIndex, reviewIndex) {
    ...
    updateEventById(eventId, updatedEvent) {
    ...
    convertEventsToEventDTOs(events) {
        const eventsAsEventDTO = events.map((event) => ({
            name: event.name,
            id: event.id,
